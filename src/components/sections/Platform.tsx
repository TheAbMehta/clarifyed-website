...
export default function Platform() {
  ...
  const circleInfo: Record<string, string> = {
    ...
    visuals: "Diagrams, graphs, diffusion-model images, and Manim-accurate explainer videos — every visual is generated by the AI and rendered inline on the whiteboard. Always mathematically exact, never hallucinated.",
    ...
    <section id="platform" ref={beyondInsightRef} className="py-28 md:py-40 px-6 text-center relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-5xl mx-auto pt-20">
        ...
        <div className="hidden md:flex justify-center items-center mb-12 relative h-80">
          ...
          <motion.button
            onClick={() => setActiveCircle(activeCircle === 'visuals' ? null : 'visuals')}
            initial={{ opacity: 0, scale: 0.8 }}
            whileInView={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.8, delay: 0.2, ease: "easeOut" }}
            viewport={{ once: true }}
            className={`absolute w-80 h-80 rounded-full border flex items-center justify-center transition-all duration-500 cursor-pointer focus-visible:outline-2 focus-visible:outline-[#e8705b] focus-visible:outline-offset-2 overflow-hidden ${activeCircle === 'visuals' ? 'border-[#e8705b] bg-[#1a1a1a]/90 backdrop-blur-3xl shadow-2xl z-50 scale-110' : 'border-[#e8705b]/30 bg-[#1a1a1a]/20 backdrop-blur-md hover:border-[#e8705b] hover:bg-[#1a1a1a]/40 z-20'}`}
          >
            <span className={`text-xl font-medium transition-colors ${activeCircle === 'visuals' ? 'text-[#e8705b]' : 'text-[#a3a3a3]'}`}>Visuals</span>
          </motion.button>
        ...
        <div className="flex md:hidden flex-col gap-4 mb-12 px-2">
          {(['whiteboard', 'visuals', 'collaboration'] as const).map((key) => (
            <motion.button
              key={key}
            ...
            >
              <span className="text-[#e8705b] text-lg font-medium">{key.charAt(0).toUpperCase() + key.slice(1)}</span>
              <AnimatePresence>
                {activeCircle === key && (
                  <motion.p
